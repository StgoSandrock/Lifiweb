import { matchInputSchema, playerInputSchema } from "@shared/lib/validation";
...
import { firebaseArtifactId, firebaseAuth, firebaseDb } from "./firebase";
...
export async function signInStaff(email: string, password: string) {
  ...
  if (!credential.user.emailVerified) {
    ...
    throw new Error("Debes verificar tu correo antes de usar el panel Staff.");
  }
  if (!(await isStaffUser(credential.user))) {
    await signOut(firebaseAuth);
    throw new Error("La cuenta inició sesión, pero no posee un rol Staff activo.");
  }
